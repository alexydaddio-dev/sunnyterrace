...
const S = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  html,body{height:100%;background:${C.sand};font-family:'DM Sans',-apple-system,sans-serif;color:${C.dark}}
  ...
  .lm{width:34px;height:34px;background:${C.terra};border-radius:9px;display:flex;align-items:center;justify-content:center}
  .li{width:18px;height:18px;border:2.5px solid white;border-radius:4px}
  .ln{font-family:'Playfair Display',serif;font-size:16px;font-weight:500;color:${C.dark}}
  .ln span{color:${C.terra}}
  .badge{font-size:11px;font-weight:500;background:${C.terraL};color:${C.terraD};padding:4px 11px;border-radius:20px}
  .title{font-size:22px;font-weight:500;margin-bottom:4px}
  .sub{font-size:14px;color:${C.muted};margin-bottom:20px;font-weight:300}
  .field{margin-bottom:14px}
  .field label{display:block;font-size:11px;font-weight:500;letter-spacing:.6px;text-transform:uppercase;color:${C.muted};margin-bottom:6px}
  .field input,.field select{width:100%;padding:14px 15px;border:1.5px solid ${C.border};border-radius:14px;font-family:'DM Sans',-apple-system,sans-serif;font-size:17px;color:${C.dark};background:${C.cream};outline:none;appearance:none;-webkit-appearance:none;transition:border-color .15s}
  .field input:focus,.field select:focus{border-color:${C.terra};background:white}
  .field input::placeholder{color:#C8C0B0}
  .row2{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px}
  .nights{background:${C.terraL};color:${C.terraD};border-radius:14px;font-size:16px;font-weight:500;padding:14px 0;text-align:center}
  .pax-row{display:flex;align-items:center;background:${C.cream};border:1.5px solid ${C.border};border-radius:14px;padding:12px 15px;margin-bottom:14px}
  .pax-label{flex:1}
  .pax-name{font-size:16px;color:${C.dark}}
  .pax-hint{font-size:12px;color:${C.muted};font-weight:300}
  .pax-ctrls{display:flex;align-items:center;gap:14px}
  .pax-btn{width:38px;height:38px;border-radius:50%;border:1.5px solid ${C.border};background:white;font-size:20px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:${C.terra};transition:all .15s}
  .pax-btn:active{background:${C.terraL};border-color:${C.terra};transform:scale(.94)}
  .pax-val{font-size:22px;font-weight:500;min-width:22px;text-align:center}
  .btn-wa{width:100%;padding:18px;background:${C.wa};color:white;border:none;border-radius:16px;font-family:'DM Sans',-apple-system,sans-serif;font-size:17px;font-weight:500;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:all .15s;margin-top:8px}
  .btn-wa:active{background:#1da851;transform:scale(.98)}
  ...
  .pin-input{width:100%;max-width:280px;padding:16px;border:1.5px solid ${C.border};border-radius:14px;font-size:24px;text-align:center;letter-spacing:8px;outline:none;background:${C.cream};font-family:'DM Sans',sans-serif}
  .pin-input:focus{border-color:${C.terra};background:white}
  .pin-error{color:#C4623A;font-size:14px;min-height:20px}
  .btn-pin{width:100%;max-width:280px;padding:16px;background:${C.terra};color:white;border:none;border-radius:14px;font-size:17px;font-weight:500;cursor:pointer}
  .btn-pin:active{background:${C.terraD}}
  .bnav{position:fixed;bottom:0;left:0;right:0;background:rgba(253,250,244,.95);border-top:1px solid ${C.border};display:flex;padding:10px 0 calc(env(safe-area-inset-bottom,0px) + 10px);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}
  ...
  .ni-label{font-size:10px;font-weight:500;color:${C.muted};letter-spacing:.3px}
  .ni.on .ni-label{color:${C.terra}}
  .ni:not(.on) .ni-icon{opacity:.45}
  .hist-item{background:${C.cream};border:1px solid ${C.border};border-radius:14px;padding:14px 16px;margin-bottom:10px;cursor:pointer}
  .hi-name{font-weight:500;font-size:15px;margin-bottom:3px}
  .hi-dates{font-size:13px;color:${C.muted};font-weight:300}
  .empty{text-align:center;color:${C.muted};font-size:14px;padding:40px 0;font-weight:300}
  .sett-h{font-size:12px;color:${C.muted};text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px;margin-top:20px}
`;

function pad(n) { return String(n).padStart(2,"0"); }
function fmtDate(s) { if (!s) return ""; const [y,m,d]=s.split("-"); return `${pad(d)}/${pad(m)}/${y}`; }
...
export default function Host() {
  ...
  const base = (baseUrl || "https://sunnytterrace.vercel.app").replace(/\/$/, "");
  const prop = propName || "Sunny Terrace";
  const cty  = city || "Nice";

  const buildMsg = () => {
    const link = `${base}/checkin?guest=${encodeURIComponent(guest)}&pax=${pax}&checkin=${checkin}&checkout=${checkout}&nights=${nights}`;
  ...
  const send = () => {
    if (!guest || !checkin || !checkout) { alert("Remplis le prénom et les dates 😊"); return; }
    const msg = buildMsg();
    const entry = { guest, checkin, checkout, pax, ts: Date.now() };
  ...
  const reuse = (r) => {
    setGuest(r.guest); setCheckin(r.checkin); setCheckout(r.checkout); setPax(r.pax || 2);
    setTab(0);
  };
  ...
  if (!auth) return (
    <>
      ...
      <div className="pin-wrap">
        ...
        <input className="pin-input" type="password" inputMode="numeric" maxLength={6}
            ...
            {hist.length === 0
              ...
              : hist.map((r, i) => (
                <div key={i} className="hist-item" onClick={() => reuse(r)}>
                  ...
                  <div className="hi-dates">{fmtDate(r.checkin)} → {fmtDate(r.checkout)} · {new Date(r.ts).toLocaleDateString("fr-FR", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</div>
                </div>
              ))
            }
          </>
        ...
        {tab === 2 && (
          <>
            ...
            <div className="field">
              ...
              <input type="url" placeholder="https://sunnytterrace.vercel.app"
